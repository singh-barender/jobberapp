...
import { BadRequestError, IAuthDocument, IEmailMessageDetails } from '@singh-barender/9-jobber-shared';
...
export async function resetPassword(req: Request, res: Response): Promise<void> {
  ...
  const existingUser: IAuthDocument | undefined = await getAuthUserByPasswordToken(token as string);
  if (!existingUser) {
